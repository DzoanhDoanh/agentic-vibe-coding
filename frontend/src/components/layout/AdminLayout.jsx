...
const AdminLayout = () => {
  ...
  if (user?.role === 'Teacher') {
    menuItems.push({ key: '/classes', icon: <BookOutlined />, label: 'Lớp Của Tôi' });
    menuItems.push({ key: '/schedule', icon: <DashboardOutlined />, label: 'Lịch Dạy' });
  }
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout>
        <Header style={{ background: '#fff', padding: '0 24px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '16px' }}>
          <div style={{ fontWeight: 'bold' }}>Xin chào, {user?.full_name} ({user?.role})</div>
